#ifndef CONTINUOUS_NON_LINEAR_OPTIMIZATION_F5_GRAYBOX_CEC_H
#define CONTINUOUS_NON_LINEAR_OPTIMIZATION_F5_GRAYBOX_CEC_H
...
template<typename T>
class f5_graybox_cec : public bounded_problem_interface<T> {
public:
...
public:
    explicit f5_graybox_cec(int dim) : Superclass(dim) {
        o = vector_t(dim);
        ...
        for(int i = global_index; i < dim; i++){
    ...
    T value(const vector_t &x) override {
        ...
        for(size_t i = 0; i < dim-global_index; i++){
            result += math.rastrigin(z_1[i], i, dim-global_index);
        }
        return result;
    }
    ...
    T delta(const vector_t &x, T fx, T delta_i, size_t i) override {
        ...
        if(id_sub_function.first){
            ...
            result = math.rastrigin(z, p, index, (index - global_index), dim - global_index);
        }
        return result - fx;
    }
};

#endif //CONTINUOUS_NON_LINEAR_OPTIMIZATION_F5_GRAYBOX_CEC_H
