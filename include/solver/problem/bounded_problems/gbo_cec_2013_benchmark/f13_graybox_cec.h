#ifndef CONTINUOUS_NON_LINEAR_OPTIMIZATION_F13_GRAYBOX_CEC_H
#define CONTINUOUS_NON_LINEAR_OPTIMIZATION_F13_GRAYBOX_CEC_H
...
template<typename T>
class f13_graybox_cec : public bounded_problem_interface<T> {
public:
...
public:
    explicit f13_graybox_cec(int dim) : Superclass(dim) {
        cec_read_files read;
        o = vector_t(dim);
    ...
    T value(const vector_t &x) override {
        ...
        for (size_t i = 0; i < number_sub_component; i++){
            ...
            switch(sub_component_size){
                case 25:{
                    math.rotateVectorConform(z, p, z_1, r25, sub_component_size, global_index, i, overlap);
                    break;
                }
                case 50:{
                    math.rotateVectorConform(z, p, z_1, r50, sub_component_size, global_index, i, overlap);
                    break;
                }
                case 100:{
                    math.rotateVectorConform(z, p, z_1, r100, sub_component_size, global_index, i, overlap);
                    break;
                }
                default:
...
#endif //CONTINUOUS_NON_LINEAR_OPTIMIZATION_F13_GRAYBOX_CEC_H
