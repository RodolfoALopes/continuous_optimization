#ifndef CONTINUOUS_NON_LINEAR_OPTIMIZATION_F15_GRAYBOX_CEC_H
#define CONTINUOUS_NON_LINEAR_OPTIMIZATION_F15_GRAYBOX_CEC_H
...
template<typename T>
class f15_graybox_cec : public bounded_problem_interface<T> {
public:
...
public:
    explicit f15_graybox_cec(int dim) : Superclass(dim) {
        o = vector_t(dim);
    ...
    T value(const vector_t &x) override {
        cec_math<T> math;
        vector_t z = x - o;
        T result = math.schwefel(z);
        return result;
    }
};

#endif //CONTINUOUS_NON_LINEAR_OPTIMIZATION_F15_GRAYBOX_CEC_H
