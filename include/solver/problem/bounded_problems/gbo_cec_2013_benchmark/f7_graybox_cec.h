#ifndef CONTINUOUS_NON_LINEAR_OPTIMIZATION_F7_GRAYBOX_CEC_H
#define CONTINUOUS_NON_LINEAR_OPTIMIZATION_F7_GRAYBOX_CEC_H
...
template<typename T>
class f7_graybox_cec : public bounded_problem_interface<T> {
public:
...
public:
    explicit f7_graybox_cec(int dim) : Superclass(dim) {
        o = vector_t(dim);
    ...
    T value(const vector_t &x) override {
        ...
        for(int i = dim -1; i >= global_index; i--){
            result += math.sphere_value(z[p[i]]);
        }
        return result;
    }
...
#endif //CONTINUOUS_NON_LINEAR_OPTIMIZATION_F7_GRAYBOX_CEC_H
